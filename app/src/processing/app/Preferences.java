...
public class Preferences {
  ...
  static protected void init(String commandLinePrefs) {
    ...
    try {
      ...
      Base.showError(null, "Could not read default settings.\n" +
                           "You'll need to reinstall happiNES-dev.", e);
    }
  ...
  public Preferences() {
    ...
    label = new JLabel("  (requires restart of happiNES-dev)");
    box.add(label);
    pain.add(box);
    ...
    if (Base.isWindows()) {
      autoAssociateBox =
        new JCheckBox("Automatically associate .pde files with happiNES-dev");
    ...
    label = new JLabel("(edit only when happiNES-dev is not running)");
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
